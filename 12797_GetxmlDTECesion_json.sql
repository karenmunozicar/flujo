...
BEGIN
   ...
   json2:=logjson(json2,'Fecha de Vencimiento FECHAVENCIMIENTO='||get_json('FECHAVENCIMIENTO',json2)||' fecha_vencimiento='||get_json_upper('fecha_vencimiento',json2)||' '||get_json('fecha_vencimiento',json2)||' FchVenc='||get_xml('FchVenc',data1));
   ...
   if(get_json('FECHAVENCIMIENTO',json2)<get_json('FECHAEMISION',json2) or get_json('FECHAVENCIMIENTO',json2)>'2500-01-01') then
	json2:=response_requests_6000('2', 'Fecha Vencimiento Inválida ','', json2);
	return json2;
   end if;


   json2 := put_json(json2,'MONTO',get_xml('MntTotal',data1));
   if get_json('RECESION',json2)='SI' then
   	json2:=recesion1_6000(json2);
...
BEGIN
	...
	id_doc:=get_json('id_doc_cesion',json2);
...
BEGIN
	...
	if (sts1<>'OK') then
   		if get_json('RECESION',json2)='SI' then
			return response_requests_6000('2', 'Falla Esquema Cesion :'||sts1,aec1, json2);
		end if;
