...
devengo['titulo'] = re.sub(r'[^\x20-\x7f\x0d\x1b\xf1\xd1]', '', json_devengo['titulo']) 
devengo['descripcion'] = re.sub(r'[^\x20-\x7f\x0d\x1b\xf1\xd1]', '', json_devengo['descripcion'])
...
if json_devengo['tipo_dte'] == 33:
    documento_devengo['monto'] = abs(json_dte['monto_neto'])
elif json_devengo['tipo_dte'] == 34:
    documento_devengo['monto'] = abs(json_dte['monto_excento'])
elif json_devengo['tipo_dte'] in [56, 61]:
    if json_dte['monto_neto'] > 0:
        documento_devengo['monto'] = abs(json_dte['monto_neto'])
    else:
        documento_devengo['monto'] = abs(json_dte['monto_excento'])
    if json_devengo['tipo_dte'] == 61:
        documento_devengo['monto'] = abs(documento_devengo['monto']) * -1
elif json_devengo['tipo_dte'] == 0:
    if abs(json_dte['monto_neto']) > 0:
        documento_devengo['monto'] = abs(json_dte['monto_neto'])
    else:
        documento_devengo['monto'] = abs(json_dte['monto_excento'])
    if json_devengo['tipo_dte'] != 61:
...
#    documento_devengo['monto'] = abs(json_dte['monto_neto']) * -1
#elif json_devengo['tipo_dte'] in [0, 61] and json_devengo['ref_tipo_dte'] == 34:
#    documento_devengo['monto'] = abs(json_dte['monto_excento']) * -1
...
if json_devengo['tipo_dte'] in [33, 34]:
    # FGE - 20200218 - Calculo del monto total otros impuestos
    monto_otros_impuestos = 0
    if json_dte['impuestos'] != None:
        for otro_impuesto in json_dte['impuestos']:
            #monto_otros_impuestos += otro_impuesto['monto']
            monto_otros_impuestos += int(otro_impuesto['MONTO'])
    impuesto_devengo = {}
    impuesto_devengo['codigo'] = '1'
    #impuesto_devengo['monto'] = str(json_dte['monto_neto'] + monto_otros_impuestos)
...
elif json_devengo['tipo_dte'] == 56 or json_devengo['tipo_dte'] == 61:
    impuestos['impuesto'] = []
    if json_devengo['contabiliza_referencia'] == 'Y':
        impuestos['contabilizaImpuestos'] = 'true'
    else:
        impuestos['contabilizaImpuestos'] = 'false'
else:
