--Publica documento
delete from isys_querys_tx where llave='12710';
insert into isys_querys_tx values ('12710',10,1,1,'select proc_procesa_cuadratura_qa_12710(''$$__XMLCOMPLETO__$$'') as __xml__',0,0,0,1,1,-1,0);

--Vamos al flujo de Servicios REST.
insert into isys_querys_tx values ('12710',90,1,8,'Salto a Flujo de Servicios REST',12760,0,0,1,1,0,0);
--Agrega Parametro al Filtro
CREATE or replace FUNCTION agrega_filtro_qa_12710(varchar,varchar,varchar) RETURNS varchar AS $$
declare
...
CREATE or replace FUNCTION estado_dte_qa_12710(varchar) RETURNS varchar AS $$
declare
	estado alias for $1;
begin
...
CREATE or replace FUNCTION procesa_exportar_datos_qa_12710(varchar) RETURNS varchar AS $$
declare
	xml1 alias for $1;
	xml2	varchar;
	query1	varchar;
	...
	--RME 20140819 busqueda avanzada
...
begin
	...
	rut_receptor1:=get_campo('avRutReceptor',xml2);
        if (is_number(rut_receptor1) is false) then
                xml2:=logapp(xml2,'Rut rut_receptor no numerico '||rut_receptor1);
		rut_receptor2:=0;
	else
	...
	for campo in select parametro from filtros_rut where rut_emisor=rut_empresa2 union select 'tipo_dte' as parametro loop
		filtro1:=agrega_filtro_qa_12710(campo.parametro,filtro1,xml2);
	end loop;
	/*
	filtro1:=agrega_filtro_qa_12710('tipo_dte',filtro1,xml2);
	filtro1:=agrega_filtro_qa_12710('PARAMETRO1',filtro1,xml2);
	filtro1:=agrega_filtro_qa_12710('PARAMETRO2',filtro1,xml2);
	filtro1:=agrega_filtro_qa_12710('PARAMETRO3',filtro1,xml2);
	filtro1:=agrega_filtro_qa_12710('PARAMETRO4',filtro1,xml2);
	filtro1:=agrega_filtro_qa_12710('PARAMETRO5',filtro1,xml2);
	*/
	xml2:=logapp(xml2,'filtros '||filtro1);
	...
	if (length(fila2)>0) then
		...
		xml2:=logapp(xml2,'Evento Detalle '||aux1);
		...
		xml2:=logapp(xml2,'Tabla Elegida '||tabla1);
	...
	if (is_number(fecha_ini1) is false) then
		...
		xml2:=logapp(xml2,'Envio Header File');
		...
		if (is_number(folio1) is false) then
	...
	xml2:=logapp(xml2,'Periodo '||periodo1||' Tabla Boleta='||tabla_boletas1);

	--RME 20140819 se arma el filtro de busqueda avanzada
	busqueda_avanzada:='';
 ...
 xml2:=logapp(xml2,'RMERME***  Busqueda Avanzada['||busqueda_avanzada||']');
	...
	if (busqueda_avanzada<>'') then
		...
		query1:='select string_agg(tipo_dte::varchar||'';''||folio::varchar||'';''||rut_receptor::varchar||''-''||modulo11(rut_receptor::varchar)||'';''||to_char(fecha_ingreso,''YYYY-MM-DD HH24:MI:SS'')||'';''||fecha_emision::varchar||'';''||monto_total::varchar'||coalesce(parametros2,'')||'||'';''||estado_dte_qa_12710(estado_sii)||'';''||estado_dte_qa_12710(estado_inter)||'';''||uri,''\n'') from (select * from '||tabla_boletas1||' where rut_emisor=$2 and tipo_dte in (33,34,46,56,61,111,110,112,39,41,43,52) '||busqueda_avanzada||' '||filtro1||' '||filtro_adicionales1||' union select * from dte_emitidos where rut_emisor=$2 and tipo_dte in (33,34,46,56,61,111,110,112,39,41,43,52) '||busqueda_avanzada||' '||filtro1||' '||filtro_adicionales1||') a' ;

xml2:=logapp(xml2,'RMERME***  query 1['||query1||']');

		execute query1 using fecha_ini1,rut_empresa2 into aux1;
		xml2:=logapp(xml2,'Exporta Detalle Folio');
	...
	--Si viene algun filtro con boletas, sacamos de otra tabla
		...
		query1:='select string_agg(tipo_dte::varchar||'';''||folio::varchar||'';''||rut_receptor::varchar||''-''||modulo11(rut_receptor::varchar)||'';''||to_char(fecha_ingreso,''YYYY-MM-DD HH24:MI:SS'')||'';''||fecha_emision::varchar||'';''||monto_total::varchar'||coalesce(parametros2,'')||'||'';''||estado_dte_qa_12710(estado_sii)||'';''||estado_dte_qa_12710(estado_inter)||'';''||uri,''\n'') from (select * from '||tabla_boletas1||' where '||dia1||'=$1::integer and rut_emisor=$2 '||filtro1||' '||filtro_adicionales1||') a';
		xml2:=logapp(xml2,'Extrae Boletas Rut='||rut_empresa2::varchar||' Dia='||fecha_ini1::varchar);
		execute query1 using fecha_ini1,rut_empresa2 into aux1;
	elsif (tabla1='dte_emitidos_errores') then
		query1:='select string_agg(tipo_dte::varchar||'';''||folio::varchar||'';''||rut_receptor::varchar||''-''||modulo11(rut_receptor::varchar)||'';''||to_char(fecha_ingreso,''YYYY-MM-DD HH24:MI:SS'')||'';''||fecha_emision::varchar||'';''||monto_total::varchar'||coalesce(parametros2,'')||'||'';''||estado_dte_qa_12710(estado)||'';''||veces||'';''||uri,''\n'') from (select * from '||tabla1||' where '||dia1||'=$1::integer and rut_emisor=$2 '||filtro1||' '||filtro_adicionales1||') a';
		xml2:=logapp(xml2,'Extrae DTE Rut='||rut_empresa2::varchar||' Dia='||fecha_ini1::varchar);
...
CREATE or replace FUNCTION procesa_detalle_qa_12710(varchar) RETURNS varchar AS $$
declare
	xml1 alias for $1;
	xml2	varchar;
	query1	varchar;
	...
	tabla_boletas1	varchar;
        ...
        rut_receptor1   varchar;
        rut_receptor2   bigint;
	avfecha_ini1	varchar;
	avfecha_fin1	varchar;
        busqueda_avanzada       varchar;
...
begin
	...
	tipo_tabla1:=get_campo('tipo_tabla',xml2);
	if (is_number(fecha_ini1) is false) then
		xml2:=logapp(xml2,'Fecha Ini no es numerico = '||fecha_ini1);
		return xml2;
	end if;
	fecha_fin1:=get_campo('fHasta',xml2);
	if (is_number(fecha_fin1) is false) then
		xml2:=logapp(xml2,'Fecha fin no es numerico = '||fecha_fin1);
		return xml2;
	end if;
	rut_empresa1:=get_campo('rut_empresa',xml2);
    	if (is_number(rut_empresa1) is false) then
	...
	*/
	...
	if (is_number(pagina1) is false) then
		xml2:=logapp(xml2,'Numero de Pagina no es numerico, vamos a pagina 1');
		pagina1:='0';
		if (tabla1='dte_emitidos_errores') then
	...
	folio1:=get_campo('avFolio',xml2);
        ...
        if (rut_receptor2>0) then
		--Se usan las fechas de la busqueda avanzada
		avfecha_ini1:=get_campo('avDesde',xml2);
        	if (is_number(avfecha_ini1) is false) then
	                xml2:=logapp(xml2,'Fecha ini avanzada no es numerico = '||avfecha_ini1);
	        end if;
		fecha_ini1:=avfecha_ini1;
		avfecha_fin1:=get_campo('avHasta',xml2);
	        if (is_number(avfecha_fin1) is false) then
	                xml2:=logapp(xml2,'Fecha Fin avanzada no es numerico = '||avfecha_fin1);
	        end if;
		fecha_fin1:=avfecha_fin1;
                busqueda_avanzada := busqueda_avanzada||' and rut_receptor='||rut_receptor2::varchar;
        end if;


xml2:=logapp(xml2,'RMERME*********** busqueda_avanzada['||busqueda_avanzada||']');


	if (busqueda_avanzada<>'') then
		filtro1:=coalesce(filtro1,'');
		filtro_adicionales1:=coalesce(filtro_adicionales1,'');
		--query1:='select *,0 as veces from (select * from dte_boletas_generica where rut_emisor=$3 '||filtro1||' '||filtro_adicionales1||' and tipo_dte in (33,34,46,56,61,111,110,112,39,41,43,52) '||busqueda_avanzada||' union select * from dte_emitidos where rut_emisor=$3 '||filtro1||' '||filtro_adicionales1||' and tipo_dte in (33,34,46,56,61,111,110,112,39,41,43,52) '||busqueda_avanzada||') a' ;
		query1:='select *,0 as veces from (select * from dte_emitidos where rut_emisor=$3 '||filtro1||' '||filtro_adicionales1||' and tipo_dte in (33,34,46,56,61,111,110,112,39,41,43,52) '||busqueda_avanzada||') a' ;
		xml2:=logapp(xml2,'Query Busqueda Avanzada');
		tabla1:='dte_emitidos';
		if (strpos(filtro_adicionales1,'tipo_dte in (''39'')')>0) then
	...
	--Si viene algun filtro con boletas, sacamos de otra tabla
		...
		xml2:=logapp(xml2,'Query Boletas');
		tabla1:='dte_emitidos';
	else
	...
	for campo in execute query1 using fecha_ini1,fecha_fin1,rut_empresa2  loop
		...
		case when tabla1='dte_emitidos_errores' then
			'<cont>'||estado_dte_qa_12710(campo.estado)||'</cont>'||chr(10)||
			'<cont>'||coalesce(campo.veces::varchar,'')||'</cont>'||chr(10)
		else
			'<cont>'||estado_dte_qa_12710(campo.estado_sii)||'</cont>'||chr(10)||
			'<cont>'||estado_dte_qa_12710(campo.estado_inter)||'</cont>'||chr(10)
		end||
	...
	xml2:=logapp(xml2,'Fecha_ini='||fecha_ini1||'Fecha_Fin='||fecha_fin1);
	xml2:=logapp(xml2,'rut='||rut_empresa2::varchar);
	xml2:=logapp(xml2,'Total_Reg='||total_reg1::varchar);
...
CREATE or replace FUNCTION procesa_consulta_cuadratura_qa_12710(varchar) RETURNS varchar AS $$
DECLARE
    xml1        alias for $1;
    xml2    varchar;
        respuesta1      varchar;
...
BEGIN
	...
	if (length(tipo_tabla1)=0) then
		...
		if not found then
			xml2:=logapp(xml2,'Tabla CUADRATURA2 no definida para ese RUT');
			return xml2;
		end if;
		xml2:=put_campo(xml2,'tipo_tabla',stTabla.tipo_tabla);
		xml2:=put_campo(xml2,'tipo_tabla_anterior',stTabla.tipo_tabla);
		--xml2:=procesa_tabla_qa_12710('tipo_dte','estado','cantidad','documentos_cuadratura','estados_cuadratura',xml2);
		xml2:=procesa_tabla_qa_12710(stTabla.columna,stTabla.fila,stTabla.agrupador,stTabla.parametro_columna,stTabla.parametro_fila,xml2,stTabla.agrupado2);
		return xml2;	
	end if;
	...
	if found then
		...
		if (stNavegacion.tipo_tabla_destino='DETALLE') then
			xml2:=procesa_detalle_qa_12710(xml2);
			return xml2;
		end if;
...
CREATE or replace FUNCTION procesa_tabla_qa_12710(varchar,varchar,varchar,varchar,varchar,varchar,varchar) RETURNS varchar AS $$
DECLARE
    fila1	alias for $1;
...
--	xml2:=put_campo(xml2,'fecha_fin','20140331');
	...
	*/
	...
	xml2:=logapp(xml2,'filtros_adicionales='||filtro_adicionales1);
	xml2:=put_campo(xml2,'FILTROS_ADICIONALES',filtro_adicionales1);

	xml2:=logapp(xml2,'tipo_tabla='||get_campo('tipo_tabla',xml2));
	...
	--Columna Tipo_DTE
	--Filas Estados
	--select array_to_json(array_agg(row_to_json(a))) into respuesta1 from (select a.tipo_dte as "Tipo DTE",case when b.estado in ('ASI','CSI') then 'Aprobado SII' when b.estado in ('RSI') then 'Rechazado SII' when b.estado='ESI' then 'Enviado SII' end as "Estado",sum(cantidad) as "Total" from (select * from indexer_hash where rut_emisor=96671750) a join indexer_estadisticas b on a.id=b.id  group by 1,2) a where a."Estado" is not null;
	--Con los filtros de busqueda
	--query1:='select array_to_json(array_agg(row_to_json(xx))) from (select c.descripcion1 as "Tipo DTE",a."Estado",a."Total" from (select a.tipo_dte as "Tipo DTE",estado_dte_qa_12710(b.estado) as "Estado",sum(cantidad) as "Total",sum(total) as "Monto" from (select * from indexer_hash where rut_emisor=$1 '||filtro1||') a join indexer_estadisticas b on a.id=b.id  group by 1,2) a join traza.config c on a."Tipo DTE"::varchar=c.evento where a."Estado" is not null) xx;';
		--(select * from indexer_estadisticas where dia>='||fecha_ini1||' and dia<='||fecha_fin1||') b on a.id=b.id) ff '||
	--if (get_campo('rut_cliente',xml2)='10185761') then
	--Hacia abajo
	if (agrupador2 is null) then
		
		/*
		query1:='select array_to_json(array_agg(row_to_json(rds))) from 
		(select descrp_filas.codigo_grupo||'||quote_literal('##')||'||descrp_filas.descripcion as '||fila1||',descrp_columnas.codigo_grupo||'||quote_literal('##')||'||descrp_columnas.descripcion as '||columna1||' ,xx.total from 
	...
	(select codigo,descripcion,codigo_grupo,orden,grilla from (select id_parametro from parametros where parametro='||quote_literal(parametro_fila1)||') a join detalle_parametros b on a.id_parametro=b.id_parametro) descrp_filas on descrp_filas.codigo=xx.fila::varchar join
	(select codigo,descripcion,codigo_grupo,orden,grilla from (select id_parametro from parametros where parametro='||quote_literal(parametro_columna1)||') a join detalle_parametros b on a.id_parametro=b.id_parametro) descrp_columnas on descrp_columnas.codigo=xx.columna::varchar order by descrp_filas.descripcion desc,descrp_columnas.orden ) rds';
		*/	
		...
		perform logfile('INDEXER F_12710 1.0 '||xml2);
		query1:='
	...
	perform logfile('INDEXER F_12710 2.0 '||xml2);
	--query1:='select array_to_json(array_agg(row_to_json(rds))) from 
	...
	resp_final1:=resp_final1||'[';
	for campo in execute query1 using rut_empresa2,tipo_fecha1 loop
		xml2:=logapp(xml2,'Recursion query1');
...
CREATE or replace FUNCTION obtiene_condicion_perfilamiento_qa_12710(bigint,bigint,varchar) RETURNS varchar AS $$
DECLARE
...
CREATE or replace FUNCTION procesa_parametros_cuadratura_qa_12710(varchar) RETURNS varchar AS $$
DECLARE
    xml1        alias for $1;
    xml2    varchar;
        respuesta1      varchar;
...
BEGIN
	...
	xml2:=logapp(xml2,'Rut Empresa Login no numerico '||rut_empresa_login1);
	...
	filtro_perfil1:=obtiene_condicion_perfilamiento_qa_12710(rut_empresa_login2,rut_cliente2,'rut_emisor');
	xml2:=logapp(xml2,'Perfil '||filtro_perfil1);
	...
	filtro_perfil1:=obtiene_condicion_perfilamiento_qa_12710(rut_empresa2,rut_cliente2,'tipo_dte');
	xml2:=logapp(xml2,'Perfil '||filtro_perfil1);
	--Sacamos los valores posibles del parametro en el indexer hash
	first1:=true;
	...
	for campo in select * from filtros_rut where rut_emisor=rut_empresa2 order by parametro loop
		...
		first1:=true;
		for campo1 in execute sql1 using rut_empresa2 loop
			--xml2:=logapp(xml2,'campo1='||campo1);
			if (first1) then
				first1:=false;
				respuesta1:=respuesta1||'<Opcion><IdOpc>TODOS</IdOpc><Texto>TODOS</Texto></Opcion>';
			end if;
...
CREATE or replace FUNCTION procesa_consulta_boletas_qa_12710(varchar) RETURNS varchar AS $$
declare
   rut_emisor1 	integer;
   xml1        alias for $1;
   xml2    varchar;
...
BEGIN
	...
	select uri into uri1 from dte_boletas where rut_emisor = rut_emisor1 and tipo_dte = tipo_dte1 and folio = folio1 and dia_emision = fechaemi1 and monto_total = monto_total1;
 	if found then
...
CREATE or replace FUNCTION ConsultaMaestroClientes_12710(varchar) RETURNS varchar AS $$
declare
   rut_emisor1  varchar;
   xml1        alias for $1;
   xml2    varchar;
...
BEGIN
        ...
        xml2:=xml1;
        xml2:=get_parametros_get(xml2);
	--cit:codigo de identificacion tributario
        rut_emisor1:=split_part(get_campo('cit',xml2),'-',1);
	if (is_number(rut_emisor1) is false) then
	        xml2:=logapp(xml2,'Rut Empresa no numerico '||rut_emisor1);
		resp1 :='Rut Empresa no numerico';
		xml2:=put_campo(xml2,'RESPUESTA','Status: 200 OK'||chr(10)||'Content-type: application/xml'||chr(10)||'Content-length: '||length(resp1)||chr(10)||chr(10)||resp1);
	        return xml2;
	end if;
	pais1:=get_campo('pais',xml2);
	if (is_number(pais1) is false) then
		xml2:=logapp(xml2,'Codigo Pais no numerico '||pais1);
		resp1 :='Codigo Pais no numerico';
		xml2:=put_campo(xml2,'RESPUESTA','Status: 200 OK'||chr(10)||'Content-type: application/xml'||chr(10)||'Content-length: '||length(resp1)||chr(10)||chr(10)||resp1);
                return xml2;
	end if;

        select query_to_xml('select clients.* from maestro_clientes clients where clients.rut_emisor ='||rut_emisor1||'',true,false,'') into resp1 ;
        if found then
           xml2:=put_campo(xml2,'RESPUESTA','Status: 200 OK'||chr(10)||'Content-type: application/xml'||chr(10)||'Content-length: '||length(resp1)
||chr(10)||chr(10)||resp1);
        else
           resp1:='NO DATA';
           xml2:=put_campo(xml2,'RESPUESTA','Status: 200 OK'||chr(10)||'Content-type: application/xml'||chr(10)||'Content-length: '||length(resp1)
...
CREATE or replace FUNCTION proc_procesa_cuadratura_qa_12710(varchar) RETURNS varchar AS $$
DECLARE
    xml1        alias for $1;
    xml2    varchar;
	input1	varchar;
...
BEGIN
    ...
    xml2:=logapp(xml2,'Se procesa Tx='||tx1);
	

	
    --Determinamos la tx a procesar
    if (tx1='Parametros') then
	xml2:=logapp(xml2,'Tx Parametros');
	xml2:=procesa_parametros_cuadratura_qa_12710(xml2);
    elsif (tx1='Consulta') then
	xml2:=logapp(xml2,'Tx Consulta');
   	...
   	--Si viene avfolio es una busqueda avanzada
		...
		xml2:=procesa_consulta_cuadratura_qa_12710(xml2);	
   	end if;
    elsif (tx1='Export') then
	xml2:=logapp(xml2,'Tx Exportar');
	xml2:=procesa_exportar_datos_qa_12710(xml2);	
    elsif (tx1='ConsultaBoletas') then
	 xml2:=logapp(xml2,'Tx ConsultaBoletas');
	xml2:=procesa_consulta_boletas_qa_12710(xml2);
    elsif (tx1='ConsultaDte') then
	 xml2:=logapp(xml2,'Tx ConsultaDte');
	xml2:=procesa_consulta_dte_12700(xml2);
    elsif (tx1='ConsultaBoletaUsuario') then
	 xml2:=logapp(xml2,'Tx ConsultaBoletaUsuario');
	xml2:=procesa_consulta_boletas_rut_12700(xml2);
    elsif (tx1='Autenticacion1')  then
	xml2:=app_movil_autenticacion(xml2);
    elsif (tx1='getEmisor')  then
	xml2:=ConsultaMaestroClientes_12710(xml2);

-------------------------------------------
--Servicio Adquira Marquet Place
    elsif (tx1='AMP_Valida_Doc') then
          xml2:=logapp(xml2,'Saltamos a flujo 12760 desde 12710');
          xml2 := put_campo(xml2,'__SECUENCIAOK__','90');
    elsif (tx1='AMP_Rechaza_Doc') then
          xml2:=logapp(xml2,'Saltamos a flujo 12760 desde 12710');
          xml2 := put_campo(xml2,'__SECUENCIAOK__','90');

-------------------------------------------
    elsif (tx1='Autenticacion')  then
	resp1:='[{"convenio":1,"razon_social":"Enaex Servicios S.A.","rut":"76041871"},{"convenio":2,"razon_social":"Cencosud Retail","rut":"81201000"},{"convenio":3,"razon_social":"ENAEX S A","rut":"90266000"},{"convenio":4,"razon_social":"Cencosud S.A.","rut":"93834000"},{"convenio":5,"razon_social":"Cencosudc004","rut":"96671750"},{"convenio":6,"razon_social":"Entel PCS telecomunicaciones S.A.","rut":"96806980"},{"convenio":7,"razon_social":"Acepta.com","rut":"96919050"},{"convenio":8,"razon_social":"CGE DISTRIBUCION","rut":"99513400"},{"convenio":9,"razon_social":"COMPANIA DE PETROLEOS DE CHILE COPEC S A","rut":"99520000"}]';
	xml2:=put_campo(xml2,'RESPUESTA','Status: 200 OK'||chr(10)||'Content-type: text/html'||chr(10)||'Content-length: '||length(resp1)||chr(10)||chr(10)||resp1);
	xml2:=logapp(xml2,'Tx Autenticacion');
   else
	xml2:=logapp(xml2,'Tx Desconocida Cuadratura');
