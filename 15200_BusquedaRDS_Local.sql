...
BEGIN
	...
	if get_json('host_canal',json2)<>'portal.casilladigital.com' then
        ...
        if(categoria1='BOLETAS_DIRECTV') then
                ...
                json2:=put_json(json2,'__FW_LOCAL__',encode_hex(' (select * from  cd_lista_mail where rut='''||rut1::varchar||''') x where dia>='||get_json('periodo_hasta_rds',json2)||' and categoria='''||categoria1||''' and size>350 '||filtro1||' order by fecha desc'));
                ...
                json2:=put_json(json2,'__FW_RDS__',encode_hex(' cd_lista_mail where rut='''||rut1::varchar||''' and categoria='''||categoria1||''' and size>350 '||filtro1||' order by fecha desc'));
        else
                --Para DEMO en empresa ACEPTA
                if get_json('rutCliente',json2)='1096919050' then 
                        json2:=put_json(json2,'__FW_LOCAL__',encode_hex(' cd_lista_mail where dia>='||get_json('periodo_hasta_rds',json2)||' and rut='''||rut1::varchar||''' and categoria='''||categoria1||''' and size>350 '||filtro1||' order by fecha desc'));
                        json2:=put_json(json2,'__FW_RDS__',encode_hex(' cd_lista_mail where rut='''||rut1::varchar||''' and categoria='''||categoria1||''' and size>350 '||filtro1||' order by fecha desc'));
                ...
                -- para la carpeta legal el usuario esta en el campo nro_cliente y se busca en otra tabla y la empresa viene en cpt
                        ...
                        json2:=put_json(json2,'__FW_LOCAL__',encode_hex(' cd_lista_carpeta_legal where dia>='||get_json('periodo_hasta_rds',json2)||' and rut='''||empresa1::varchar||''' and nro_cliente='''||rut1::varchar||''' and categoria='''||categoria1||''' and size>350 '||filtro1||' order by fecha desc'));
                        json2:=put_json(json2,'__FW_RDS__',encode_hex(' cd_lista_carpeta_legal where rut='''||empresa1::varchar||''' and nro_cliente='''||rut1::varchar||''' and categoria='''||categoria1||''' and size>350 '||filtro1||' order by fecha desc'));
