...
then
   ...
   if (length(aux)>0) then
    ...
    if (is_number(split_part(rut_libro1,'-',1)) is false) then
	--Graba en la traza
    ...
    execute 'insert into libros_data (rut_emisor, periodo, fecha_emision, dia, tipo_libro, tipo_operacion, tipo_envio, total_dtes, monto_total, uri ) values ('||split_part(rut_libro1,'-',1)::integer||','||quote_literal(periodo1)||',now(),to_char(now(),''YYYYMMDD'')::integer,'||quote_literal(tipo_libro1)||','||quote_literal(tipo_operacion1)||','||quote_literal(tipo_envio1)||',0,0,'||quote_literal(uri1)||');';
