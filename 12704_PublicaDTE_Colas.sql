...
--                	xml2 := logapp(xml2, 'MANDATO_CICLO: "' || get_campo('MANDATO_CICLO', xml_c) || '"');
			...
			--DAO 20200713 - Si no es ENTEL(nro_cuenta) buscamos por rut,tipo,folio
			if aux1!='' then
				tipo_aux1:=get_campo('TIPO_DTE',xml2);
                                folio_aux1:=get_campo('FOLIO',xml2);
                                select id_referencia_facturacion into v_id_referencia_facturacion from referencia_facturacion where rut_emisor=v_rut_emisor and rut_receptor=v_rut_receptor and periodo=v_periodo1 and coalesce(nro_cuenta,'')<>'' limit 1;
                                if not found then
                                        select id_referencia_facturacion into v_id_referencia_facturacion from referencia_facturacion where rut_emisor=v_rut_emisor and rut_receptor=v_rut_receptor and periodo=v_periodo1 and tipo_dte=tipo_aux1 and folio=folio_aux1 limit 1;
                                end if;
			else
				...
				for record1 in select tipo_ref, folio_ref, fecha_ref, razon_ref from detalle_referencia_facturacion where id_referencia_facturacion = v_id_referencia_facturacion loop
					...
					--AVC 20180927
                                ...
                                xml2 := logapp(xml2, 'Referencias a agregar: ' || xml_referencias||' RUT_USUARIO_ACCION='||get_campo('RUT_USUARIO_ACCION',xml2));
				...
				if strpos(get_campo('XML_FLAGS',xml2),'RETENIDO1VEZ-')=0 then
					...
					--DAO 20200713 si ya fue retenido una vez y no encontramos las referencias, emitimos
