...
--	xml2:=logapp(xml2,'entrada_hex '||get_campo('INPUT',xml2));
	...
	if( is_number(rutEmiSDV) is false or is_number(rutRecSDV) is false or is_number(tipodoc1) is false or is_number(folio1) is false or is_numeric(montototal1) is false) then
	...
	--FAY-DAO 20200611 buscamos por rut tipo folio debido a algunas inconsistencias del RC
	...
	if not flag1 or (stDteRecibido.rut_receptor<>rutRecSDV::integer or stDteRecibido.fecha_emision<>split_part(fechaEmitido1,' ',1) or stDteRecibido.monto_total<>montototal1::bigint) then
		...
		EXCEPTION WHEN OTHERS THEN
			xml2:=logapp(xml2,'Falla Insert datos invalidos');
			respuesta1:='<b>FALLA</b>';
           		status1:='Status: 444 NK'||chr(10)||
		...
		xml3:=put_campo(xml3,'COMENTARIO_TRAZA','Se obtiene Fecha en Reporte Consolidado del SII. (Fecha Reporte '||to_char(now(),'YYYY-MM-DD HH24:MI')||')');
		...
		if (flag1 and coalesce(stDteRecibido.estado_sii,'') in ('ACEPTADO_POR_EL_SII','ACEPTADO_CON_REPAROS_POR_EL_SII')) then
			xml2:=logapp(xml2,'DTE ya esta en dte recibidos con otro monto,fecha,rut_rec');
                        perform logfile_rc('DTE ya esta en dte recibidos con otro monto,fecha,rut_rec');
			xml3:=put_campo(xml3,'COMENTARIO_TRAZA','Ya existe otro documento con otros datos(Recepción Consolidada)');
			xml3:=graba_bitacora(xml3,'ERROR_DTE');
			xml2:=logapp(xml2,get_campo('_LOG_',xml3));
                        --Salgo
                        respuesta1:='DTE ya se encuentra aceptado';
                        status1:='Status: 200 OK'||chr(10)||
                                 'Content-type: application/json;charset=UTF-8;'||chr(10)||
                         'Content-length: '||length(respuesta1)||chr(10)||chr(10)||respuesta1;
                         xml2 := put_campo(xml2,'__SOCKET_RESPONSE__','RESPUESTA');
