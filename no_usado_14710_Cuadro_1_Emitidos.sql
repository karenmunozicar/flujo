...
delete from isys_querys_tx where llave='14710';
...
insert into isys_querys_tx values ('14710',5,9,1,'select pivote_14710(''$$__JSONCOMPLETO__$$''::json) as __json__',0,0,0,1,1,-1,0);

insert into isys_querys_tx values ('14710',10,1,9,'<EJECUTA0>14711</EJECUTA0><EJECUTA1>14712</EJECUTA1><EJECUTA2>14713</EJECUTA2><TIMEOUT>15</TIMEOUT>',0,0,0,1,1,20,20);
--insert into isys_querys_tx values ('14710',10,1,9,'$$MULTI_PROCESO$$',0,0,0,1,1,20,20);

--Pivote para armar respuesta
insert into isys_querys_tx values ('14710',20,9,1,'select valida_respuesta_14710(''$$__JSONCOMPLETO__$$''::json) as __json__',0,0,0,1,1,-1,0);
--FUNCION valida_respuesta_cuadro2_14710
--FUNCION valida_respuesta_cuadro1_excepciones_14710
--FUNCION select_suma_emision_actual_14710 
--Voy a la base de los emitidos Redshift
insert into isys_querys_tx values ('14711',10,9,1,'select select_cuadro_1_emitidos_redshift_14711(''$$__JSONCOMPLETO__$$''::json) as __json__',0,0,0,1,1,-1,0);

--Voy a la base de los emitidos Redshift
insert into isys_querys_tx values ('14712',10,9,1,'select select_cuadro_1_boletas_redshift_14712(''$$__JSONCOMPLETO__$$''::json) as __json__',0,0,0,1,1,-1,0);

--Voy a la base de las excepciones Redshift
insert into isys_querys_tx values ('14713',10,9,1,'select select_cuadro_1_excepciones_redshift_14713(''$$__JSONCOMPLETO__$$''::json) as __json__',0,0,0,1,1,-1,0);

CREATE or replace FUNCTION pivote_14710(json) RETURNS json AS $$
declare
        json2   json;
        json1   alias for $1;
	grupo_tot1	varchar;
...
begin
	...
	if (v_fecha_inicio>=to_char(now(),'YYYYMMDD')::integer and v_fecha_fin>=to_char(now(),'YYYYMMDD')::integer and get_json('TIPO_FECHA',json2)<>'Emision') then
		...
		json2:=valida_respuesta_14710(json2);
		return json2;
	end if;
	...
	if(get_json('__CUADRO__',json2)='1') then
		...
		json2 := put_json(json2,'grupo_tot','');
		--Vamos al multihilo
		--json2 := put_json(json2,'MULTI_PROCESO','<EJECUTA0>14711</EJECUTA0><EJECUTA1>14712</EJECUTA1><EJECUTA2>14713</EJECUTA2><TIMEOUT>15</TIMEOUT>');
		...
		if (get_json('evento_tot',json2) in ('Error de Esquema','Folios Duplicados','En Espera','Documentos Reprocesados','Documentos Repetidos','Rechazados por Acepta')) then
			json2:=select_cuadro_1_excepciones_redshift_14713(json2);	
			json2 := put_json(json2,'__SECUENCIAOK__','20');
			return json2;
        	elsif (grupo_tot1 in ('documentos_nacionales','documentos_exportacion')) then
			json2:=select_cuadro_1_emitidos_redshift_14711(json2);
			json2 := put_json(json2,'__SECUENCIAOK__','20');
			return json2;
        	elsif (grupo_tot1='documentos_boletas') then
			json2:=select_cuadro_1_boletas_redshift_14712(json2);
			json2 := put_json(json2,'__SECUENCIAOK__','20');
			return json2;
		else
			json2:=select_cuadro_1_emitidos_redshift_14711(json2);
...
CREATE or replace FUNCTION valida_respuesta_cuadro2_14710(json) RETURNS json AS $$
DECLARE
        json2   json;
        json1   alias for $1;
        lista_emi       json;
...
BEGIN
	...
	exception when others then
		...
		if (grupo_tot1<>'documentos_boletas') then
                	json5:=select_suma_emision_actual_14710(json2);
...
CREATE or replace FUNCTION valida_respuesta_cuadro1_excepciones_14710(json) RETURNS json AS $$
DECLARE
	json2	json;
	json1	alias for $1;
    	json_emi	json;
...
BEGIN
	...
	exception when others then
		...
		json2:=put_json(json2,'flag_excepciones','SI');
                json5:=select_suma_emision_actual_14710(json2);
		--perform logfile('FAY select_suma_emision_actual_14710='||json5);
                if (json5 is not null) then
                        lista_emi:=suma_json2(lista_emi,json5,'["count"]','["tipo_dte","estado"]');
                        --Para las excepciones, sume diferente
                        --if (get_json('tipo_tx',json2)='select_cuadro_1_excepciones') then
...
CREATE or replace FUNCTION valida_respuesta_14710(json) RETURNS json AS $$
DECLARE
	json2	json;
	json1	alias for $1;
    	json_emi	json;
...
BEGIN
	...
	--	return valida_respuesta_cuadro1_excepciones_14710(json2);
	--Cuadro 2
	--elsif (get_json('tipo_tx',json2)<>'select_documentos_x_rut_resumen_totalizados') then
	if(get_json('__CUADRO__',json2)='2') then
		json2:=logjson(json2,'Ejecuta valida_respuesta_cuadro2_14710');
		return valida_respuesta_cuadro2_14710(json2);
	end if;
	...
	json5:=put_json(json5,'criterio_busqueda',get_json('criterio_busqueda',json2));
        json4:=put_json_list(json4,json5);
        json3:=valida_respuesta_cuadro1_excepciones_14710(json2);
...
CREATE or replace FUNCTION select_suma_emision_actual_14710(json) RETURNS json AS $$
DECLARE
    json2                json;
    json1                alias for $1;
...
--CUADROS EMITIDOS
CREATE or replace FUNCTION select_cuadro_1_emitidos_redshift_14711(json) RETURNS json AS $$
DECLARE
    json2                json;
    json1                alias for $1;
    v_rut_emisor        integer;
...
BEGIN
	...
	--perform logfile('FAY '||json3::varchar);
...
--CUADROS EXCEPCIONES
CREATE or replace FUNCTION select_cuadro_1_excepciones_redshift_14713(json) RETURNS json AS $$
DECLARE
    json2                json;
    json1                alias for $1;
    v_rut_emisor        integer;
...
BEGIN
	...
	json4:=logjson(json4,'Busqueda Emitidos '||json3::varchar);
        if (get_json('STATUS',json3)<>'OK') then
		json4:=logjson(json4,'Falla Busqueda de Emitidos en BASE_REDSHIFT_EMITIDOS');
		json4:=put_json(json4,'EXCEPCIONES','[]');
		return json4;
        end if;
	...
	json4:=put_json(json4,'EXCEPCIONES',lista1);
	return json4;
END;
$$ LANGUAGE plpgsql;

--CUADROS BOLETAS
CREATE or replace FUNCTION select_cuadro_1_boletas_redshift_14712(json) RETURNS json AS $$
DECLARE
    json2                json;
    json1                alias for $1;
    v_rut_emisor        integer;
...
BEGIN
	...
	json2:=logjson(json2,'Ejecuta select_cuadro_1_boletas_redshift_14711');
        v_rut_emisor:=get_json('rutCliente',json2)::integer;
        v_rut_usuario:=get_json('rutUsuario',json2)::integer;
