...
BEGIN
    ...
    if (get_campo('ESTADO_ENVIO_ERP',xml2)='OK') then
		...
		insert into documentos_recibidos_x_enviar_erp_historica (fecha,rut,canal,uri,id,reintentos,fec_ult_reintento,fec_envio_erp) select *, now() from documentos_recibidos_x_enviar_erp  where id=id1::bigint;
