...
insert into isys_querys_tx values ('12794',10,8009,1,'select emitir_documento_firmado_12794(''$$__JSONCOMPLETO__$$'') as __json__',0,0,0,1,1,-1,0);

--insert into isys_querys_tx values ('12794',15,8009,1,'select emitir_documento_pivote_v2_12794(''$$__JSONCOMPLETO__$$'') as __json__',0,0,0,1,1,-1,0);
...
BEGIN
	...
	--Si tengo un folio o intento de folio en id_temporal_gestor_folios para la misma sesion en estado=1, significa que fallo el anterior (borrado o emision),  por ende usamos este id para la transaccion
		select * into stCorrelativo from id_temporal_gestor_folios where rut=rut1 and tipo_dte=tipo_dte1 and estado in (1,3) and sesion=get_json('session_id',json2) order by id limit 1 for update;
		if found then
...
BEGIN
	...
	/*if exists(select 1 from tmp3) is false and get_json('__TBK__', json2)<>'SI' then
		--get_json('rutUsuario',json2)='17597643' then
		json2:=logjson(json2,'emitir_documento_pivote_v2_12794');
		insert into tmp3 values(get_json('session_id',json2));
		return emitir_documento_pivote_v2_12794(json2);
        end if;*/
	return emitir_documento_pivote_v2_12794(json2);
