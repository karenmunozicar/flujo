delete from isys_querys_tx where llave='2117';

--Va por el API Base_PG_WEB puerto 8009
insert into isys_querys_tx values ('2117',10,9,1,'select ws_sk_consulta_2117(''$$__XMLCOMPLETO__$$'') as __xml__',0,0,0,1,1,-1,0);
insert into isys_querys_tx values ('2117',20,1,10,'$$SCRIPT$$',0,0,0,1,1,30,30);

insert into isys_querys_tx values ('2117',30,1,1,'select ws_sk_consulta_resp_2117(''$$__XMLCOMPLETO__$$'') as __xml__',0,0,0,1,1,-1,0);

CREATE or replace FUNCTION ws_sk_consulta_2117(varchar) RETURNS varchar AS $$
DECLARE
    xml1		alias for $1;
    xml2		varchar;
    data1		varchar;
...
BEGIN 
    ...
    rut_emisor1:=get_xml('tem:RutEmisor',input1);
    tipo_dte1:=get_xml('tem:TipoDte',input1);
    folio1:=get_xml('tem:Folio',input1);	

    xml2:=logapp(xml2,'rutemisor1-->'||rut_emisor1||' tipodte1-->'||tipo_dte1||' folio1-->'||folio1);
    resp_head:='<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">';
    resp_body:='<soap:Body>';
    resp_foot:='</soap:Body>
</soap:Envelope>';


        --Valida que sea uno de los rut de SKC
        if rut_emisor1::integer not in (76030514,76167834, 84196300,96777170,96937550,76692840,76003473,76410610,76053361,76047121,76176602,96517990) then
                resp_body:=resp_body || 'Emisor No Autorizado a Consultar' ;
                resp1:= resp_head || resp_body || resp_foot;
        ...
        if tipo_dte1 in ('39','41') then
                resp1:= resp_head || resp_body || 'Consultas Boleta no Permitida' ||resp_foot;
                xml2:=put_campo(xml2,'RESPUESTA','Status: 200 OK'||chr(10)||'Content-type: text/html'||chr(10)||'Content-length: '||length(resp1)||chr(10)||chr(10)||resp1);
		return xml2;
        else
                --busco es estado del documento
                select estado_sii, uri into estado1, uri1 from dte_emitidos where rut_emisor = rut_emisor1::integer and tipo_dte = tipo_dte1::integer and folio = folio1::integer;
                if estado1 ='RECHAZADO_POR_EL_SII' then
                        evento1:='RSI';
                elsif  estado1 ='ACEPTADO_CON_REPAROS_POR_EL_SII' then
                        evento1:='CSI';
                elsif estado1 ='ACEPTADO_POR_EL_SII' then
                        evento1 := 'ASI';
                else
                        evento1 :='PENDIENTE_SII';
                        descr1:='';
                end if;

                if estado1<>'' then
                        t_traza1:=get_tabla_traza(uri1);
                        execute 'select comentario1 from '||t_traza1||' where uri=$1 and evento=$2 ' into descr1 using uri1, evento1;
                end if;

                select row_to_json(a) into resp1 from ( select rut_emisor1 as rut,tipo_dte1 as tipo_Doc ,folio1 as Folio , estado1  estado, descr1 as descripcion ) as a ;
        end if;
        if found then
           resp_body:=resp_body||resp1;
           resp1 := resp_head || resp_body || resp_foot;
           xml2:=put_campo(xml2,'RESPUESTA','Status: 200 OK'||chr(10)||'Content-type: text/html'||chr(10)||'Content-length: '||length(resp1)||chr(10)||chr(10)||resp1);
       else
           resp1:=resp_head || resp_body || 'NO DATA' ||resp_foot;
           xml2:=put_campo(xml2,'RESPUESTA','Status: 200 OK'||chr(10)||'Content-type: text/html'||chr(10)||'Content-length: '||length(resp1)||chr(10)||chr(10)||resp1);
        end if;

	xml2:=logapp(xml2,'SK: RESPUESTA-->'||get_campo('RESPUESTA',xml2));
