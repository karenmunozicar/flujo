...
insert into isys_querys_tx values ('3000',5,1914,1,'select graba_mail_s3_3000(''$$__XMLCOMPLETO__$$'') as __xml__',0,0,0,1,1,-1,0);
...
insert into isys_querys_tx values ('3000',10,29,1,'select inserta_mail_3000(''$$__JSONCOMPLETO__$$''::json) as __json__',0,0,0,1,1,-1,0);
...
BEGIN
	...
	if strpos(adicional1,'<ERROR_ORIGEN>')>0 then
		flag1:='SI';
		BEGIN
			query1:='insert into send_mail_cabecera_'||to_char(now(),'YYMM')||' (id,fecha_ingreso,dia,msg_id,estado,de,para,cc,bcc,dominio,uri,categoria,rut_owner,ciclo,adicional,documento,json_msg_id) values ('||i::varchar||','''||now()::varchar||'''::timestamp,'||to_char(now(),'YYYYMMDD')||','''||msg_id1||''','''||get_xml('ERROR_ORIGEN',adicional1)||''','''||from1||''','''||to1||''','''||get_json('cc',json2)||''','''||get_json('bcc',json2)||''','''||lower(split_part(split_part(to1,'@',2),'>',1))||''','''||get_json('URI_IN',json2)||''','''||get_json('CATEGORIA',json2)||''',nullif('''||rut_owner1||''','''')::bigint,nullif('''||coalesce(ciclo1::varchar,'')||''','''')::integer,'''||coalesce(adicional1,'')||''','''||coalesce(uri1,'')||''',nullif('''||coalesce(json_msgid1::varchar,'')||''','''')::json ) returning id';
		...
		BEGIN
			query1:='insert into send_mail_cabecera_'||to_char(now(),'YYMM')||' (id,fecha_ingreso,dia,msg_id,estado,de,para,cc,bcc,dominio,uri,categoria,rut_owner,ciclo,adicional,documento,json_msg_id) values ('||i::varchar||','''||now()::varchar||'''::timestamp,'||to_char(now(),'YYYYMMDD')||','''||msg_id1||''',''POR_ENVIAR'','''||from1||''','''||to1||''','''||get_json('cc',json2)||''','''||get_json('bcc',json2)||''','''||lower(split_part(split_part(to1,'@',2),'>',1))||''','''||get_json('URI_IN',json2)||''','''||get_json('CATEGORIA',json2)||''',nullif('''||rut_owner1||''','''')::bigint,nullif('''||coalesce(ciclo1::varchar,'')||''','''')::integer,'''||coalesce(adicional1,'')||''','''||coalesce(uri1,'')||''',nullif('''||coalesce(json_msgid1::varchar,'')||''','''')::json ) returning id';
		EXCEPTION WHEN OTHERS THEN
			json2:=logjson(json2,'Falla insert tabla mensual '||SQLERRM);
			query1:='insert into send_mail_cabecera (id,fecha_ingreso,dia,msg_id,estado,de,para,cc,bcc,dominio,uri,categoria,rut_owner,ciclo,adicional,documento,json_msg_id) values ('||i::varchar||','''||now()::varchar||'''::timestamp,'||to_char(now(),'YYYYMMDD')||','''||msg_id1||''',''POR_ENVIAR'','''||from1||''','''||to1||''','''||get_json('cc',json2)||''','''||get_json('bcc',json2)||''','''||lower(split_part(split_part(to1,'@',2),'>',1))||''','''||get_json('URI_IN',json2)||''','''||get_json('CATEGORIA',json2)||''',nullif('''||rut_owner1||''','''')::bigint,nullif('''||coalesce(ciclo1::varchar,'')||''','''')::integer,'''||coalesce(adicional1,'')||''','''||coalesce(uri1,'')||''',nullif('''||coalesce(json_msgid1::varchar,'')||''','''')::json ) returning id';
		END;
