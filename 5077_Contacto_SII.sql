delete from isys_querys_tx where llave='5077';

insert into isys_querys_tx values ('5077',5,1,16,'LOG_JSON',0,0,0,1,1,10,10);
-- Prepara llamada al AML
insert into isys_querys_tx values ('5077',10,1,1,'select contacto_sii_5077(''$$__JSONCOMPLETO__$$'') as __json__',0,0,0,1,1,-1,0);
insert into isys_querys_tx values ('5077',50,1,1,'Publica Boleta Honorario',112704,0,0,0,0,60,60);
insert into isys_querys_tx values ('5077',60,1,1,'select procesa_publicacion_boleta_hon_5077(''$$__JSONCOMPLETO__$$'') as __json__',0,0,0,1,1,-1,0);	

CREATE or replace FUNCTION contacto_sii_5077(json) RETURNS json AS $$
DECLARE
    json1       alias for $1;
        json2   json;
        json3   json;
...
BEGIN
        json2:=json1;
	--Remplazamos los ; 
	json2:=logjson(json2,'Entra a 5077');
	...
	if(get_json('categoria',json2)='BORRAR') then
		...
		perform logfile('5077= '||coalesce(folio1,'')||','||coalesce(rut_emisor1,'')||','||coalesce(rut1,'')||','||coalesce(tipo_dte1,''));
		...
		--Parseamos los Datos con el XML
/*
		...
		json2:=logjson(json2,'DAO_DATOS_BOLETA_HON '||split_part(split_part(boleta1,'RUT: ',1),'N Â° ',2));
...
*/
		rut_emisor1:=get_xml('rutEmisor',boleta_xml1);
		rut_receptor1:=get_xml('rutReceptor',boleta_xml1);
		monto1:=get_xml('totalHonorarios',boleta_xml1);
		giro1:=get_xml('actividadEconomica',boleta_xml1);
		fono1:=get_xml('telefonoEmisor',boleta_xml1);
		direccion1:=get_xml('domicilioReceptor',boleta_xml1);
		tipo_dte1:=get_xml('tipodoc',boleta_xml1);
		fecha_emi1:=get_xml('FechorGen',boleta_xml1);
		begin
                        fecha_emi2:=to_timestamp(fecha_emi1,'DD/MM/YYYY HH24:MI');
                        fecha_emi1:=to_char(fecha_emi2,'YYYY-MM-DD');
                exception when others then
		...
		folio1:=get_xml('numeroBoleta',boleta_xml1);
		uri1_pdf:=get_json('uri_pdf',json2);
		uri1:='http://'||(select dominio from maestro_clientes where rut_emisor=rut_receptor1::bigint)||to_char(now(),'YYMM')||'.acepta.com/v01/'||genera_uri2(rut_emisor1,tipo_dte1,folio1,fecha_emi1,monto1,'R');
...
CREATE or replace FUNCTION procesa_publicacion_boleta_hon_5077(json) RETURNS json AS $$
DECLARE
	json1       alias for $1;
	json2	json;
	boleta1	varchar;
...
BEGIN
        json2:=json1;
        --Remplazamos los ;
        json2:=logjson(json2,'Procesa Pub Boleta Honorario');
        json2:=replace(json2::varchar,'\u0006',';')::json;
        json2:=put_json(json2,'__SECUENCIAOK__','0');
        --json2:=logjson(json2,'JSON INPUT='||json2::varchar);

	if get_json('__PUBLICADO_OK__',json2) <>'SI' then
		json2:=logjson(json2,'Falla Publicacion Boleta Honorario URI '||get_json('URI_IN',json2));
		insert into boleta_honorario_sin_publicar (data) values (json2::varchar);
		return json2;
	end if;
	...
	if(rut_emisor1='' or rut_receptor1='' or monto1='' or is_number(rut_emisor1) is false or is_number(rut_receptor1) is false or is_number(monto1) is false or is_number(folio1) is false or is_number(tipo_dte1)) then
        	return response_requests_6000('2','Error Boleta emisor['||rut_emisor1||'] receptor['||rut_receptor1||'] monto['||monto1||'] folio['||folio1||'] ','',json2);
        end if;

	--Se busca si ya esta grabada la Boleta
	select * into campo from dte_boletas_honorario_generica where rut_emisor=rut_emisor1::integer and tipo_dte=tipo_dte1::integer and folio=folio1::integer;
	if not found then
		insert into dte_boletas_honorario_generica(fecha_ingreso,mes,dia,tipo_dte,folio,fecha_emision,mes_emision,dia_emision,rut_emisor,rut_receptor,monto_total,estado,uri,estado_sii,data_dte)
                values(now(),to_char(now(),'YYYYMM')::integer,to_char(now(),'YYYYMMDD')::integer,tipo_dte1::integer,folio1::integer,fecha_emi1,to_char(fecha_emi2,'YYYYMM')::integer,to_char(fecha_emi2,'YYYYMMDD')::integer,rut_emisor1::integer,rut_receptor1::integer,monto1::integer,'BOLETA_GRABADA_OK',get_json('URI_IN',json2),'BOLETA_GRABADA_OK','<Giro>'||coalesce(giro1,'')||'</Giro><Direccion>'||coalesce(direccion1,'')||'</Direccion><Fono>'||coalesce(fono1,'')||'</Fono>');
                return response_requests_6000('1','Datos OK rut_emisor1='||rut_emisor1||' rut_receptor1='||rut_receptor1||' monto1='||monto1||' fecha_emi2='||fecha_emi2,'',json2);
        else
                return response_requests_6000('1','Boleta ya registrada rut_emisor1='||rut_emisor1||' rut_receptor1='||rut_receptor1||' monto1='||monto1||' fecha_emi2='||fecha_emi2,'',json2);
        end if;
