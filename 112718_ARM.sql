delete from isys_querys_tx where llave='112718';

--Sacamos la URI
--insert into isys_querys_tx values ('112718',5,1,1,'select proc_get_uri_data_112718(''$$__XMLCOMPLETO__$$'') as __xml__',0,0,0,1,1,-1,0);

--Todo ARM se publica
--Se publica desde l 8010
--insert into isys_querys_tx values ('112718',10,1,8,'Publica DTE',12704,0,0,0,0,20,20);
insert into isys_querys_tx values ('112718',5,19,1,'select proc_get_input_uri_112718(''$$__XMLCOMPLETO__$$'') as __xml__',0,0,0,1,1,-1,0);

--Primero que hacemos el publicar DTE
insert into isys_querys_tx values ('112718',10,1,8,'Publica DTE',112704,0,0,0,0,20,20);


-- Prepara llamada al AML
insert into isys_querys_tx values ('112718',20,1,1,'select proc_procesa_input_arm_112718(''$$__XMLCOMPLETO__$$'') as __xml__',0,0,0,1,1,-1,0); 

--Llamada al Flujo de Publicacion
--Llamada al AML directo SGCI
--insert into isys_querys_tx values ('112718',25,1,8,'Llamada Publica DTE',12704,0,0,0,0,30,30);
--insert into isys_querys_tx values ('112718',30,1,2,'Llamada directo al AML',14000,102,101,0,0,40,40);
insert into isys_querys_tx values ('112718',31,1,8,'Escribe ARM EDTE',112780,0,0,0,0,40,40);

--Llamada al AML directo SGGI CGE
--insert into isys_querys_tx values ('112718',32,1,8,'Llamada Publica DTE',12704,0,0,0,0,35,35);
insert into isys_querys_tx values ('112718',35,1,2,'Llamada directo al AML CGE',14009,102,101,0,0,40,40);



--Respuesta del AML
insert into isys_querys_tx values ('112718',40,1,1,'select proc_procesa_respuesta_arm_112718(''$$__XMLCOMPLETO__$$'') as __xml__ ',0,0,0,1,1,-1,0);

--Se ejecuta en la 88
insert into isys_querys_tx values ('112718',1000,19,1,'select sp_procesa_respuesta_cola_motor88(''$$__XMLCOMPLETO__$$'') as __xml__',0,0,0,1,1,0,0);


CREATE or replace FUNCTION proc_get_input_uri_112718(varchar) RETURNS varchar AS $$
DECLARE
	xml1        alias for $1;
        xml2    varchar;
	depot1	varchar;
BEGIN
	...
	if(get_campo('INPUT',xml2)='') then
		depot1:=get_input_almacen('{"uri":"'||get_campo('URI_IN',xml2)||'"}');	
		xml2:=put_campo(xml2,'INPUT',depot1);
		xml2:=put_campo(xml2,'CONTENT_LENGTH',length(depot1)::varchar);
	end if;
	return xml2;
END;
$$ LANGUAGE plpgsql;
...
/*
CREATE or replace FUNCTION proc_get_uri_data_112718(varchar) RETURNS varchar AS $$
DECLARE
	data_hex1	varchar;
	data1	varchar;
	uri_in1	varchar;
...
CREATE or replace FUNCTION proc_procesa_input_arm_112718(varchar) RETURNS varchar AS $$
DECLARE
    xml1        alias for $1;
        xml2    varchar;
    data1       varchar;
...
BEGIN
   ...
   if (length(split_part(split_part(input1,'filename="',2),'"',1))>0) then
	   xml2 := put_campo(xml2,'URI_IN',split_part(split_part(input1,'filename="',2),'"',1));
   else
	   xml2 := put_campo(xml2,'URI_IN',split_part(split_part(input1,'"custodium-uri">',2),'</',1));
   end if;
   ...
   if (is_number(rut_emisor1) is false or is_number(tipo_dte1) is false or is_number(folio1) is false) then
	...
	--Corrijo la basura para grabar evento en traza
	   ...
	   --Si ya tenemos estado_reclamo, ignoramos el ARM
	   if (coalesce(stRec.estado_reclamo,'')<>'') then
        	respuesta1:='URL(True): '||get_campo('URI_IN',xml2)||chr(10)||'ARM ya reclamado';
	   ...
	   if (stRec.uri_arm is not null) then
        	...
        	xml2 := logapp(xml2,'DTE '||stRec.uri ||' ya tiene ARM registrado URI='||get_campo('URI_IN',xml2));
		if (length(get_campo('__COLA_MOTOR__',xml2))>0) then
			--xml2:=sp_procesa_respuesta_cola_motor(xml2);
			--Si estoy reprocesando y esta en los rechazados los borro
			md5_input1:=md5(get_campo('INPUT',xml2));
...
CREATE or replace FUNCTION proc_procesa_respuesta_arm_112718(varchar) RETURNS varchar AS $$
DECLARE
    xml1        alias for $1;
    xml2        varchar;
    data1       varchar;
...
BEGIN
    ...
    if (get_campo('__EDTE_ARM_OK__',xml2)='BORRAR') then
	     texto_resp1:='Se borra ARM';
             status1:='Status: 200 OK'||chr(10)||
                 'Content-type: text/html'||chr(10)||
                 'Content-Location: '||get_campo('URI_IN',xml2)||chr(10)||
