...
BEGIN
	...
	xml3:=graba_bitacora(xml3,get_campo('EVENTO',xml3));
...
BEGIN
	...
	if not found then
		if(now()-get_json('FECHA_INGRESO_COLA',json2)::timestamp>interval '3 days') then
			...
			json2:=logjson(json2,'Rut Receptor no se encuentra en maestro_clientes '||rut1::varchar||' Se borra de la cola (5 dias)');
        		...
        		select * into campo from maestro_clientes where rut_emisor=rut_emisor1::integer;
			if not found then
	...
	--Identificamos si es un NAR o un CRT
/*	if(strpos(xml_dte1,encode('<ResultadoDTE','hex'))>0) then

		categoria1:='NAR';		
		json2:=logjson(json2,'Ensobro NAR Recibido '||uri1);
	elsif(strpos(xml_dte1,encode('<EnvioRecibos','hex'))>0) then
		categoria1:='ARM';		
		json2:=logjson(json2,'Ensobro ARM Recibido '||uri1);
	else
		--RecepcionDTE
		categoria1:='CRT';		
		json2:=logjson(json2,'Ensobro CRT Recibido '||uri1);
	end if;
	json2:=put_json(json2,'CATEGORIA_MAIL',categoria1);
	rut1:=split_part(get_xml_hex1('RutRecibe',xml_dte1),'-',1);
        rut_emisor1:=split_part(get_xml_hex1('RutResponde',xml_dte1),'-',1);
