delete from isys_querys_tx where llave='12728';

-- Prepara llamada al AML
insert into isys_querys_tx values ('12728',20,1,1,'select proc_procesa_input_12728(''$$__XMLCOMPLETO__$$'') as __xml__',0,0,0,1,1,-1,0); 

--Llamada al Flujo de Publicacion
--Llamada al AML directo SGCI
--insert into isys_querys_tx values ('12728',25,1,8,'Llamada Publica DTE',12704,0,0,0,0,30,30);
insert into isys_querys_tx values ('12728',30,1,2,'Llamada directo al AML',14000,102,101,0,0,40,40);
insert into isys_querys_tx values ('12728',31,1,2,'Llamada directo al AML',14001,102,101,0,0,40,40);
insert into isys_querys_tx values ('12728',32,1,2,'Llamada directo al AML',14002,102,101,0,0,40,40);
insert into isys_querys_tx values ('12728',33,1,2,'Llamada directo al AML',14003,102,101,0,0,40,40);
insert into isys_querys_tx values ('12728',34,1,2,'Llamada directo al AML',14004,102,101,0,0,40,40);
insert into isys_querys_tx values ('12728',35,1,2,'Llamada directo al AML',14005,102,101,0,0,40,40);
insert into isys_querys_tx values ('12728',36,1,2,'Llamada directo al AML',14006,102,101,0,0,40,40);
insert into isys_querys_tx values ('12728',37,1,2,'Llamada directo al AML',14007,102,101,0,0,40,40);
insert into isys_querys_tx values ('12728',38,1,2,'Llamada directo al AML',14008,102,101,0,0,40,40);
insert into isys_querys_tx values ('12728',39,1,2,'Llamada directo al AML CGE',14009,102,101,0,0,40,40);

--Respuesta del AML
insert into isys_querys_tx values ('12728',40,1,1,'select proc_procesa_respuesta_12728(''$$__XMLCOMPLETO__$$'') as __xml__ ',0,0,0,1,1,-1,0);


CREATE or replace FUNCTION proc_procesa_input_12728(varchar) RETURNS varchar AS $$
DECLARE
    xml1        alias for $1;
        xml2    varchar;
    data1       varchar;
...
BEGIN
   ...
   xml2 := logapp(xml2,'DATA CA4AEC='||replace(input1,'\012',chr(10)));
   ...
   xml2:=logapp(xml2,'CA4AEC rut_emisor1='||rut_emisor1||' tipo_dte1='||tipo_dte1||' folio1='||folio1||' monto1='||monto1||' fecha_emi1='||fecha_emi1||' URI='||get_campo('URI_IN',xml2));
...
CREATE or replace FUNCTION proc_procesa_respuesta_12728(varchar) RETURNS varchar AS $$
DECLARE
    xml1        alias for $1;
    xml2        varchar;
    data1       varchar;
...
BEGIN
    ...
    xml2:=logapp(xml2,'CA4AEC: Texto Respuesta AML='||resp1);
    ...
    if (strpos(resp1,'200 OK')>0 and strpos(resp1,texto_resp1)>0) then
        xml2 := logapp(xml2,'CA4AEC: FOLIO='||get_campo('FOLIO',xml2)||' RUT_EMISOR='||get_campo('RUT_EMISOR',xml2)||' TIPO_DTE='||get_campo('TIPO_DTE',xml2)||' Respuesta OK Aml');
        ...
        xml2 := logapp(xml2,'CA4AEC: FOLIO='||get_campo('FOLIO',xml2)||' RUT_EMISOR='||get_campo('RUT_EMISOR',xml2)||' TIPO_DTE='||get_campo('TIPO_DTE',xml2)||' Falla Respuesta AML');
