...
begin
	...
	*/
	...
	if (is_number(folio1)) then
		query1:='select string_agg(tipo_dte::varchar||'';''||folio::varchar||'';''||rut_receptor::varchar||''-''||modulo11(rut_receptor::varchar)||'';''||to_char(fecha_ingreso,''YYYY-MM-DD HH24:MI:SS'')||'';''||fecha_emision::varchar||'';''||monto_total::varchar'||coalesce(parametros2,'')||'||'';''||estado_dte_12709(estado_sii)||'';''||estado_dte_12709(estado_inter)||'';''||uri,''\n'') from (select * from dte_boletas where rut_emisor=$2 and tipo_dte in (33,34,46,56,61,111,110,112,39,41,43,52) and folio='||folio1||' '||filtro1||' '||filtro_adicionales1||' union select * from dte_emitidos where rut_emisor=$2 and tipo_dte in (33,34,46,56,61,111,110,112,39,41,43,52) and folio='||folio1||' '||filtro1||' '||filtro_adicionales1||') a' ;
	...
	--Si viene algun filtro con boletas, sacamos de otra tabla
		...
		query1:='select string_agg(tipo_dte::varchar||'';''||folio::varchar||'';''||rut_receptor::varchar||''-''||modulo11(rut_receptor::varchar)||'';''||to_char(fecha_ingreso,''YYYY-MM-DD HH24:MI:SS'')||'';''||fecha_emision::varchar||'';''||monto_total::varchar'||coalesce(parametros2,'')||'||'';''||estado_dte_12709(estado_sii)||'';''||estado_dte_12709(estado_inter)||'';''||uri,''\n'') from (select * from dte_boletas where '||dia1||'=$1::integer and rut_emisor=$2 '||filtro1||' '||filtro_adicionales1||') a';
...
begin
	...
	*/
	...
	if (is_number(folio1)) then
		...
		query1:='select *,0 as veces from (select * from dte_boletas where rut_emisor=$3 '||filtro1||' '||filtro_adicionales1||' and tipo_dte in (33,34,46,56,61,111,110,112,39,41,43,52) and folio='||folio1||' union select * from dte_emitidos where rut_emisor=$3 '||filtro1||' '||filtro_adicionales1||' and tipo_dte in (33,34,46,56,61,111,110,112,39,41,43,52) and folio='||folio1||') a' ;
		xml2:=logapp(xml2,'Query Busqueda Avanzada');
		tabla1:='dte_emitidos';

	--Si viene algun filtro con boletas, sacamos de otra tabla
	elsif (strpos(filtro_adicionales1,'tipo_dte in (''39'')')>0) then
		query1:='select *,0 as veces from dte_boletas where '||dia1||'>=$1::integer and '||dia1||'<=$2::integer and rut_emisor=$3 '||filtro1||' '||filtro_adicionales1||' offset '||pagina1||' limit 100';
...
BEGIN
	...
	xml2:=logapp(xml2,'Tx Consulta');
    	...
    	if (length(get_campo('avFolio',xml2))>0) then
		xml2:=procesa_detalle_12709(xml2);
		return xml2;
	else
